...
def singleSquareMovesToEdge(index: int) -> List[int]:
    ...
    NumOfSquaresToEdge.extend(
        [
            ...
            down_right,
            down_left,
            knight_up_right_vert,
            knight_up_right_hori,
            knight_up_left_vert,
            knight_up_left_hori,
            knight_down_right_vert,
            knight_down_right_hori,
            knight_down_left_vert,
            knight_down_left_hori,
        ]
    )
...
def collision(board: BoardState, piece: Piece, index: int, offset: int) -> List[bool]:
    ...
    if 0 <= index + offset < 64:
        if piece.color != boardPiece.color:
...
def generateLegalRookMoves(
    ...
    for offset in moveset:
        ...
        legalMoves += getDefaultMovesForOffset(
            board, piece, pieceIndex, offset, numberOfSquaresToEdge
        )
...
def generateLegalBlackPawnMoves(
    ...
    for offset in moveset:
        ...
        if offset in [7, 9]:
            if (
                len(previousMovesList) > 0
                and previousMovesList[-1][0].name == "Pawn"
                and (previousMovesList[-1][2] - previousMovesList[-1][1]) / 8 == 2
                and previousMovesList[-1][2] + 8 == pieceIndex + offset
            ):
            ...
            if pieceIndex + offset < 64 and board[pieceIndex + offset] == "none":
                ...
                legalMoves += getDefaultMovesForOffset(
                    board, piece, pieceIndex, offset, min(numberOfSquaresToEdge, 1)
                )
        else:
            legalMoves += getDefaultMovesForOffset(
                board,
                ...
                pieceIndex,
                offset,
                min(numberOfSquaresToEdge, 2 if piece.firstmove else 1),
                False,
            )
...
def generateLegalKingMoves(
    ...
    for offset in moveset:
        ...
        if (
            ...
            legalMoves += getDefaultMovesForOffset(
                board, piece, pieceIndex, offset, min(numberOfSquaresToEdge, 2)
            )
        elif (
            offset == 1
            and next((rook for rook in eligableRooks if piece.position < rook.position), None)
...
def GenerateLegalMoves(
    ...
    if (
        piece.name == "King"
        and piece.position + 2 in legalmoves
        and piece.position + 1 not in legalmoves
    ):
